...
So when it comes to uploading a file via POST which I got stuck with, and when I begin thinking how the binary data is really put in a request message, I remember there is an optional 'BODY' part we can rely on, which usually appears in a response message when you want to GET some data from the server. 
